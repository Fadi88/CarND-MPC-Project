...
public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    fg[0] = 0.0;
    //cost function of certain path error 
    for(int i = 0 ; i < N ; i++){
    ...
    for (int t = 0; t < N - 2; t++) {
      fg[0] += weight_delta * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t] , 2);
      fg[0] += weight_a     * CppAD::pow(vars[a_start + t + 1]     - vars[a_start + t]     , 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //states there no much control we can have here since this is the real world so we let the 
  //values float as it comes 
